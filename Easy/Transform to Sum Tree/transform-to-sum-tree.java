...
class Solution {
    public void toSumTree(Node root) {
        ...
        // Add the original value to the updated value of the current node
       // root.data += originalValue;
    }

    private int sumOfSubtrees(Node node) {
        if (node == null) {
            return 0;
        }
        return node.data + sumOfSubtrees(node.left) + sumOfSubtrees(node.right);
    }
}
